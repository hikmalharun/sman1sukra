...
class Absen_siswa extends CI_Controller
{
    ...
    public function print_report()
    {
        ...
        if ($kode == 'tanggal') {
            $html .= '<th style="width: 10%;">Jam Masuk</th><th style="width: 10%;">Jam Pulang</th><th>Terlambat</th>';
        } else {
            // Kolom hari di bulan
        ...
        foreach ($data_siswa as $siswa) {
            ...
            if ($kode == 'tanggal') {
                ...
                if ($jam > '06.30.00') {
                    $html .= '<td style="text-align:center; color: red;">'.$jam.'</td><td></td>';
                } else {
                    $html .= '<td style="text-align:center;">'.$jam.'</td><td></td>';
                }
    ...
    public function ganti_foto()
    {
        ...
        $config['file_name'] = $nisn.'.jpg'; // Simpan sebagai NISN.jpg

        // Delete existing file if it exists
        $existing_file = './assets/img/foto_siswa/'.$nisn.'.jpg';
        ...
        if ($this->upload->do_upload('foto')) {
            ...
            $nama_foto = $nisn.'.jpg'; // Simpan sebagai NISN.jpg

            // Update data foto di database
            $this->db->set('foto', $nama_foto);
            $this->db->where('nisn', $nisn);
            $this->db->update('tbl_data_siswa_poe_ibu');

            $this->session->set_flashdata('notifikasi', 'Foto berhasil diubah.');
        } else {
            $this->session->set_flashdata('notifikasi', 'Gagal mengunggah foto. Pastikan file berformat JPG, JPEG, atau PNG dan tidak lebih dari 2MB.');
        }
